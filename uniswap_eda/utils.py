...
def tick_to_price(tick: str, decimals0: str, decimals1: str) -> float:
    """
    Calculates price as ratio of units of token0 per units of token1, token1Price
    as provided in the pool contract. Somewhat confusingly, this is the quantity
    of token1 that must be paid provided to receive a single unit of token0 in
    exchange.
    """

    return 1.0001 ** (float(tick)) / 10 ** (int(decimals1) - int(decimals0))


def sqrt_price_to_tick(sqrt_price: str, decimals0: str, decimals1: str) -> float:
    """
    Calculates tick from square root price as 64x96 fixed-point number and the
    specified decimal places for the tokens.
    """

    return (((int(sqrt_price)) ** 2) / 2 ** (96 * 2)) * 10 ** (
        int(decimals0) - int(decimals1)
    )
